...
public class IndexModel : PageModel
{
    ...
    [BindProperty(SupportsGet = true)]
    public string app_name { get; set; } = "Doordash";

    [BindProperty(SupportsGet = true)]
    public double MPG { get; set; } = 23.00;

    [BindProperty(SupportsGet = true)]
    public double offer { get; set; } = 10.00;

    [BindProperty(SupportsGet = true)]
    public double distance_mi { get; set; } = 15.00;
    ...
    public async Task<IActionResult> OnGetStats()
    {
        ...
        var results = (await task1).ToList().SingleOrDefault();
        var results2 = (await task2).ToList().SingleOrDefault();
...
".Trim();
        ...
        int rows_affected = await connection.ExecuteAsync(query, entry);
    ...
    )
    {
        ...
        offers = (
            await connection.QueryAsync<GigOffer>(
            ...
            .If(
                ...
                items =>
                    items.Where(offer =>
                        offer.ToString().Contains(Query, StringComparison.OrdinalIgnoreCase)
                    )
            )
            // filter using regular expressions:
            .If(regex_match, items => items.Where(offer => QueryAsRegex.IsMatch(offer.ToString())))
            .ToList();
